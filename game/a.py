...
class Drone:
    ...
    def update(self, dt):
        ...
        if keys[pygame.K_UP]:
            self.left_thrust += self.thruster_amp
            self.right_thrust += self.thruster_amp
        ...
        total_thrust = self.left_thrust + self.right_thrust
        thrust_angle = self.rotation

        self.x_acceleration = total_thrust * math.cos(thrust_angle) 
        self.y_acceleration = total_thrust * math.sin(thrust_angle) - self.gravity 

        lever_arm = self.image.get_width() / 10  # Adjust as needed
        torque = total_thrust * lever_arm  
